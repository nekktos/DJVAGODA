...
# Убиваем ВСЁ ДЕРЕВО процессов, а не только запущенный нами.
#
# Игра поднимается с --host и занимает UDP-порт 24545 — тот самый, на котором
# работают все автопроверки. Kill() убивал только процесс-обёртку, живой Godot
# оставался держать порт, и следующий полный прогон падал ВЕСЬ: двадцать семь
# наборов подряд с «сессия не поднялась». Полчаса на разбор того, что сломал
# сам же.
