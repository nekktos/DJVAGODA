...
func _run() -> void:
	...
	# Досиживаем, пока клиент не закончит СВОЮ половину. Она ждёт чужой магии до
	# двадцати секунд, и уйдя раньше, хост уносит с собой её персонажа: клиент
	# видит «персонажа нет» и валится на ровном месте. Один раз так и вышло.
...
func _run_client(_me: Node3D) -> void:
	...
	for i in 40:
		...
		if not Net.active:
			# Хост ушёл раньше нас. Это не «магия не доехала», и говорить надо
			# именно это: иначе следующий разбор начнётся с поиска ошибки в
			# репликации, которой нет.
			note("сессия закрылась раньше, чем половина клиента доработала")
			break
