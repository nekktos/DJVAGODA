...
## Как выйти из зоны стороны наружу, по порядку точек.
##
## Бойцы ходят ПО ПРЯМОЙ, без обхода препятствий — это ограничение Этапа 6, и
## для отряда живого игрока оно терпимо: командир ведёт их сам и обходит стены
## глазами. Отряду ИИ вести некому. Первый же трёхминутный прогон показал, чем
## это кончается: стража вышла в набег, упёрлась в южную стену дворца в ста
## тридцати метрах от базы и простояла там до конца прогона, пока якорь строя
## ушёл вперёд один.
##
## Точки описывают единственный проход наружу и держатся ЗДЕСЬ, рядом с той
## геометрией, которая их и создаёт: сдвинешь стену или пандус — сразу видно,
## что маршрут надо поправить.
##
## У эльфов список пуст: их поселение стоит на открытой поляне без стен.
const SORTIE_PATH := {
	# Форт злодея: проём в южной стене (см. _build_villain, разрыв между
	# сегментами при f.x ± 50).
	0: [Vector3(-300.0, 0.0, 348.0)],
	1: [],
	# Дворец: сначала проём в южной стене (_build_emperor, разрыв при c.x ± 75),
	# затем спуск по пандусу — шестиметровый уступ иначе непроходим.
	2: [Vector3(300.0, 6.0, -158.0), Vector3(300.0, 0.0, -48.0)],
}
