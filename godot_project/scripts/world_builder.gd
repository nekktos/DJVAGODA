...
## Насколько вход вынесен ПЕРЕД скалой шахты. Сама скала — глыба 50 метров в
## поперечнике, и её центр лежит внутри камня: подойти можно только ко входу.
## Число нужно и снаружи — набор «проходимость» проверяет, что до входа есть
## дорога, — поэтому оно здесь, а не вписано в постройку.
const MINE_ENTRANCE_AHEAD := 26.0
...
func _build_mine() -> void:
	...
	_harvestable(_box(g, Vector3(m.x, 10.0, m.y), Vector3(50.0, 20.0, 50.0), "rock"),
		RES.Kind.IRON, 40)
	_box(g, Vector3(m.x, 4.0, m.y + MINE_ENTRANCE_AHEAD), Vector3(14.0, 8.0, 6.0),
		"dark_stone")  # вход
