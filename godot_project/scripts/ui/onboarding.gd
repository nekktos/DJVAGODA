...
func _live_text(step: Dictionary, world: Node3D, me: Node3D) -> String:
	...
	if int(goal.palace_owner) == int(me.faction):
		# НЕ «держи его». Первая версия этой строки так и говорила, и живой
		# игрок сразу спросил: «а зачем мне держать дворец дальше?» —
		# справедливо, потому что незачем. Победа объявляется в тот миг, когда
		# захват дошёл до ста процентов, и обратно НЕ отыгрывается
		# (`objective.gd::_maybe_declare`, GDD 7): отдашь точку — победа всё
		# равно твоя. Подсказка, зовущая стеречь то, что стеречь не нужно, —
		# такая же ложь, как и молчание.
		return "ДВОРЕЦ ВЗЯТ, ПОБЕДА объявлена и назад не отыгрывается. Дальше партия идёт песочницей"
