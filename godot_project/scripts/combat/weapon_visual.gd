...
## Где у каждой модели РУКОЯТЬ — по местной оси Z, вдоль которой оружие лежит.
##
## ЗАЧЕМ. `_build` рисует оружие от середины: у меча клинок уходит в +Z, а
## рукоять лежит на -0.62. Пока оружие вешали по габаритам руки (`attach`), это
## гасил сдвиг `GRIP_FORWARD`. Скелетный путь (`attach_at`) вешает узел прямо в
## кость `Weapon.R` БЕЗ сдвига — и кисть оказывалась ровно на середине клинка:
## эльф держал меч за лезвие. У лука и арбалета рукоять и так в начале
## координат, поэтому заметили только на мече.
##
## Число здесь — это «на сколько вынести оружие вперёд, чтобы рукоять пришлась
## на кость», то есть МИНУС координата рукояти в `_build`. Меняешь `_build` —
## меняй и здесь: эти две таблицы обязаны читаться вместе.
const GRIP_Z := {
	WEAPONS.Kind.BOW: 0.0,        # рукоять лука уже в начале координат
	WEAPONS.Kind.CROSSBOW: 0.24,  # рукоять ложа на -0.24
	WEAPONS.Kind.SPELL: 0.20,     # посох держат ниже середины
	WEAPONS.Kind.AXE: 0.28,       # короткое древко, хват у нижнего конца
	WEAPONS.Kind.HAMMER: 0.40,    # длинное древко, хват у самого низа
}
## Меч и всё, чего нет в таблице: рукоять в `_build` лежит на -0.62.
const GRIP_Z_DEFAULT := 0.62
...
static func attach_at(mount: Node3D, kind: int, previous: Node3D, tier: int = 0) -> Node3D:
	...
	# Вынести вперёд ровно настолько, чтобы на кость пришлась РУКОЯТЬ, а не
	# середина клинка (см. `GRIP_Z`).
	holder.position = Vector3(0.0, 0.0, GRIP_Z.get(kind, GRIP_Z_DEFAULT))
	return holder
