...
const SYNONYMS := {
	...
	# одной позы.
	# `Sit` — из своей кузницы, и это первая поза, которой в покупных паках нет
	# вовсе. Пока её не было, безногий «полз» по стойке смирно, просто опущенный
	# под землю. Порядок важен: свои модели берут настоящую позу, покупные
	# по-прежнему откатываются на `Idle` — регрессии для них нет.
	"sit": ["sit", "Sit", "Idle"],
	"wheelchair-sit": ["wheelchair-sit", "Sit", "sit", "Idle"],
	"wheelchair-move-forward": ["wheelchair-move-forward", "Walk"],
	# Вздрагивание от попадания. У Quaternius называется с опечаткой в имени
	# (`RecieveHit`), и это их имя, а не наше, — переименовывать нельзя, иначе
	# перестанут находиться их же модели.
	"hit": ["hit", "RecieveHit"],
	...
	#
	# `Punch` остался последним запасным путём. Он был нужен, пока эльф ходил в
	# покупном Ranger, где анимации замаха нет вовсе; свои модели из кузницы
	# несут настоящий `Sword_Attack`, и до него дело не доходит. Убирать не
	# стали: список — это перевод на ЛЮБУЮ модель, а не только на текущую.
	"attack-melee-right": [
